...
async function fetching(route, method, id, title, content) {
  await fetch(`http://localhost:4000/notes/${route}`, {
    method: `${method}`,
    headers: {
      "Content-type": "application/json; charset=UTF-8",
    },
...
function del(id) {
  ...
  document.getElementById('close').onclick = () => {
    fetch(`http://localhost:4000/notes/delete/${getId}`, {
      method: `DELETE`,
      headers: {
        "Content-type": "application/json; charset=UTF-8",
      },
...
async function edit(id) {
  getId = id;
  const response = await fetch(`http://localhost:4000/notes/singleNote/${getId}`, {
    method: `GET`,
    headers: {
      "Content-type": "application/json; charset=UTF-8",
    },
